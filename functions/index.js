...
exports.searchStudents = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    if (req.method !== 'POST') {
      return res.status(405).json({ error: 'Método não permitido' });
    }
    ...
    try {
      ...
      const results = allStudents
        .filter(student => 
          ...
          student.code.includes(lowercasedTerm)
        )
        .slice(0, 5);
      ...
      return res.status(500).json({ error: 'Erro interno do servidor ao buscar alunos.' });
    }
  });
});