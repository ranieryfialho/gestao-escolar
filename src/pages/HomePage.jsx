...
function HomePage() {
    ...
    useEffect(() => {
        ...
        const canSeeInactive = ['coordenador', 'auxiliar_coordenacao', 'diretor'].includes(userRole);
        if (canSeeInactive) {
    ...
    const getVisibleCardsData = () => {
        ...
        const cardDefinitions = {
            ...
            retuf: { key: "retuf-report", title: "Relatório RETUF", value: "Acessar", icon: FileSpreadsheet, to: "/retuf", bgColor: "bg-blue-100", textColor: "text-blue-600" },
            laboratorio: { key: "lab-support", title: "Laboratório de Apoio", value: "Acessar", icon: Beaker, to: "/laboratorio", bgColor: "bg-lime-100", textColor: "text-lime-600" },
            eventos: { key: "events-management", title: "Gestão de Eventos", value: "Gerenciar", icon: Ticket, to: "/eventos", bgColor: "bg-rose-100", textColor: "text-rose-600" },
            treinamentoBasico: { key: "tb-classes", title: "Treinamentos Básicos", value: tbClassesCount, icon: BookOpenCheck, to: "/frequencia", bgColor: "bg-indigo-100", textColor: "text-indigo-600" },
            cursosExtras: { key: "extra-courses", title: "Cursos Extras", value: extraCoursesCount, icon: BookCopy, to: "/frequencia", bgColor: "bg-purple-100", textColor: "text-purple-600" },
            cursosEmentas: { key: "courses-syllabi", title: "Cursos e Ementas", value: "Visualizar", icon: BookImage, to: "/cursos", bgColor: "bg-pink-100", textColor: "text-pink-600" },
            calculadora: { key: "replacement-calculator", title: "Calculadora de Reposição", value: "Calcular", icon: Calculator, to: "/calculadora", bgColor: "bg-sky-100", textColor: "text-sky-600" },
            frequenciaNexus: { key: "frequencia-nexus", title: "Frequência Nexus", value: "Acessar", icon: CalendarPlus, to: "/frequencia-nexus", bgColor: "bg-cyan-100", textColor: "text-cyan-600" },
            gerarContrato: { key: "generate-contract", title: "Gerar Contrato", value: "Criar", icon: FileText, to: "/gerar-contrato", bgColor: "bg-violet-100", textColor: "text-violet-600" },
        ...
        if (userRole !== 'comercial' && userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.turmasFinalizando);
        if (userRole !== 'comercial' && userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.modulosFinalizando);
        if (['coordenador', 'auxiliar_coordenacao', 'diretor'].includes(userRole)) orderedCards.push(cardDefinitions.retuf);
        if (userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.laboratorio);
        if (userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.eventos);
        if (userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.treinamentoBasico);
        if (userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.cursosExtras);
        if (userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.cursosEmentas);
        if (userRole !== 'professor_nexus') orderedCards.push(cardDefinitions.calculadora);
          ...
          <div className="p-4 md:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {professorCards.map(card => (
                ...
                <Link
                    to="/boletim"
                    state={{ isNexusUser: true, lockedSchoolId: "GEYs70ghHbdAm9oeE8hu" }}
                ...
                >
                    <div className="bg-white p-4 rounded-lg shadow-md hover:shadow-xl transition-shadow flex flex-col items-center text-center h-full justify-center">
                        ...
                        <div>
                            ...
                            <h3 className="text-sm font-medium text-gray-500 mt-1">Boletim Nexus</h3>
                        </div>
                    </div>
                </Link>
            </div>
    ...
    const remainingLabels = allModulesInChart.filter((mod) => !moduleOrder.includes(mod) && mod !== "Sem Módulos");
    const finalLabels = [...sortedLabels, ...remainingLabels];
    const finalData = finalLabels.map((label) => moduleCounts[label]);
    const moduleChartData = { labels: finalLabels, datasets: [{ label: "Nº de Turmas", data: finalData, backgroundColor: "rgba(59, 130, 246, 0.6)", borderColor: "rgba(59, 130, 246, 1)", borderWidth: 1 }]};
    const handleBarChartClick = (event, elements) => {
        if (!elements || elements.length === 0) return;
        const { index } = elements[0];
        navigate("/mapa-turmas", { state: { filter: "module", moduleName: moduleChartData.labels[index] }});
    };
    const moduleChartOptions = { responsive: true, maintainAspectRatio: false, onClick: handleBarChartClick, onHover: (event, chartElement) => { event.native.target.style.cursor = chartElement[0] ? "pointer" : "default" }, plugins: { legend: { display: false }, title: { display: true, text: "Turmas por Módulo Atual" }}, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 }}}};
    const gradeStatusCounts = activeClasses.reduce( (acc, turma) => {
        turma.students?.forEach((student) => {
            if (student.grades) {
                Object.values(student.grades).forEach(gradeInfo => {
                    const grade = parseFloat(typeof gradeInfo === "object" ? gradeInfo.finalGrade : gradeInfo);
    ...
    const doughnutChartData = { labels: ["Notas Acima da Média", "Notas Abaixo da Média"], datasets: [{ data: [gradeStatusCounts.green, gradeStatusCounts.red], backgroundColor: ["#4CAF50", "#F44336"], hoverBackgroundColor: ["#66BB6A", "#EF5350"], borderColor: "#fff", borderWidth: 2 }]};
    const doughnutChartOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom" }, title: { display: true, text: "Desempenho Geral de Notas" }}};
    ...
    return (
        <div className="p-4 md:p-8">
            ...
            {rows.map((row, index) => (
                <div key={index} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-6">
                    {row.map(card => (
                        <StatCard
                            key={card.key}
                            title={card.title}
